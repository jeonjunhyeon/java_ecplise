...
public class RandomGameView implements ActionListener{
	// 선언부
	// 동쪽에 붙일 속지 선언 및 생성
	JPanel jp_east = new JPanel(); //Fragment - 안드로이드에서 똑같이 사용함
	...
	JScrollPane jsp_diplay = new JScrollPane(jta_display,
											 JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
											 JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
	// 남쪽에 그려질 속지 선언 및 생성 - 여기에는 버튼 10개를 붙일 거임 - 일단 방만 10개 만들어났따.
	JPanel jp_south = new JPanel();
	JButton jbtns[] = new JButton[10]; // 딱!! 방만 있는 것이다.
	// 참조형 타입의 초기화는 null로 할 수 있다.
	JButton jbtn    = null;
	// 문자열을 담을 수 있는 방 10개를 만들고, 숫자를 0~9까지 초기화 하였음.
	...
	public void initDisplay() {
		...
		// JFrame의 배치, 레이아웃을 정하는데 setLayout으로 정할 수 있다.
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==jbtn_exit) {
			System.exit(0); // 자바 가상 머신과의 연결고리가 끊어져 종료가 된다.
