...
//ActionListener인 나는 버튼을 눌렀을 때 일어날 일에 대해서 알 수 없다. 
	//결정할 수 없다.
	//모르겠다.
	BaseBallGameView bbgView = null;
	BaseBallGameLogic bbgLogic = null;
	
	public BaseBallGameEvent(BaseBallGameView baseBallGameView, BaseBallGameLogic bbgLogic) {
		this.bbgView = bbgView;
	}
	public BaseBallGameEvent(BaseBallGameView baseBallGameView, BaseBallGameLogic bbgLogic, BaseBallGameView bbgView) {
		this.bbgView = bbgView;
		this.bbgLogic = bbgLogic;
	}

	//인터페이스는 반드시 구현체 클래스가 필요하다.
	//왜 그래야하냐면 actionPerformed라는 추상메소드를 구현할 수 있으니까!
	@Override
	public void actionPerformed(ActionEvent e) {
		//bbgView가 널이 아니기 위해서는 반드시 BaseBallGameEvent1의 생성자를 경유해야 한다.(3/8 KeyPoint)
