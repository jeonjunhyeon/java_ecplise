...
public class BaseBallGameView {//소통의 시작
	...
	//컴파일 타임에 인스턴스화 하기
	// - 이른 인스턴스화이다.(멀티스레드에서는 안전하지 않다/메모리의 과다사용으로 서버 다운)
	// ★ 파라미터를 갖는 생성자가 1개라도 있으면 어떠한 생성자도 제공하지 않음. ★ -> 이것을 통해서 문법적인 문제를 해결할 수 있음.
	BaseBallGameLogic bbgLogic = new BaseBallGameLogic();
	BaseBallGameEvent bbgEvent = new BaseBallGameEvent(this, bbgLogic); // 이것이 객체 주입(의존성 주입)
