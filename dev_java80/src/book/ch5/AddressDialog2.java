...
public class AddressDialog2 extends JDialog implements ActionListener {
	...
	// sql문을 사용할 때 오라클에서 사용해보고 에러 없으면 붙여서 사용할 것
	/*
	 * INSERT INTO dept(deptno, dname, loc)
	 * VALUES(53, '개발팀','포항')
	 */
	// 문자열을 처리할 때 StringBuilder를 사용하거나, StringBuffer를 사용하면 된다.
	StringBuffer sql_ins2 = new StringBuffer(); // 멀티스래드에서 안전, 속도는 느림!
	StringBuilder sql_ins = new StringBuilder(); // 싱글스레드에서 안전, 속도는 빠름!
	/*
	 * UPDATE dept
       SET loc = '서울'
       WHERE deptno = 52
    ...
    public void actionPerformed(ActionEvent e) {
       ...
       if("처리".equals(command)) {
       	...
       	if(dVO==null) {//입력일 때
       		...
       		pdVO.setDeptno(Integer.parseInt(getJtf_deptno())); // 이렇게 사용할 때 NumberFormatException을 조심해야 한다.
       		pdVO.setDname(getJtf_dname());
       		pdVO.setLoc(getJtf_loc());
       		// String 클래스 사용 시 주의해야 함
       		...
       		try {
				...
				JOptionPane.showMessageDialog(aBook.jf, "Exception"+e.toString());
			}
       	}
       	else {//수정일 때
       		DeptVO pdVO = new DeptVO();
       		...
       		pdVO.setDname(getJtf_dname());
       		pdVO.setLoc(getJtf_loc());
       		sql_upd.append("UPDATE ?");
       		sql_upd.append("SET loc = ?");
       		sql_upd.append("WHERE deptno = 52");
       		try {
       			upstmt = con.prepareStatement(sql_upd.toString());
       			int i = 0; //물음표 갯수만큼 필요하며, 시작은 1부터 입니다.
